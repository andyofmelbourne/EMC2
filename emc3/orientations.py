...
def get_rotation_matrices(
    ...
    if queue is None or context is None:
        from .utils_cl import opencl_init
        cl_stuff = opencl_init()
        queue = cl_stuff['queue']
        context = cl_stuff['context']
    ...
    if dimensions == 3 and rotation_order > 0:
        ...
        raise ValueError(
            f'could not reconsile dimension {dimensions} '
            f'and rotation_order {rotation_order}')
...
def calculate_rotation_matrices(Mrot, M_in_plane, M_sphere, queue, context):
    cl_code = cl.Program(context, r"""
        ...
        // s_p2 = sin(phi_2), s_p = sin(phi), s_t = sin(theta)
        //[-s_2 s_t + c_p c_2 c_t, -s_2 c_p c_t - s_t c_2, s_p c_t],
        //[ s_2 c_t + s_t c_p c_2, -s_2 s_t c_p + c_2 c_t, s_p s_t],
        //[-s_p c_2, s_p s_2, c_p]])

        __kernel void calculate_rotation_matrix (
            global float *R, const int M_in_plane, const int M_sphere)
        {
        ...
        Ro[0] = -sin(phi_2) * sin(theta)
                      + cos(phi) * cos(phi_2) * cos(theta);
        Ro[1] = -sin(phi_2)*cos(phi)*cos(theta)
                      - sin(theta)*cos(phi_2) ;
    ...
    for t in tqdm.tqdm(
        range(1), desc='pre-calculating rotation matrices', disable=quiet
    ):
        cl_code.calculate_rotation_matrix(
            queue, (Mrot,), None, R_cl.data,
            np.int32(M_in_plane), np.int32(M_sphere)
        )
...
def calculate_rotation_matrices_2D(Mrot, queue, context):
    cl_code = cl.Program(context, r"""
        ...
        __kernel void calculate_rotation_matrix (
                         global float *R, const int M_in_plane
        )
        {
        // this is the rotation index r
        int r = get_global_id(0);

        float theta = 2. * M_PI * r / M_in_plane + (float)0.37576737726076753;
    ...
    for t in tqdm.tqdm(
        ...
        cl_code.calculate_rotation_matrix(
            queue, (Mrot,), None, R_cl.data, np.int32(Mrot)
        )
...
def get_rotations_3D(M, queue, context):
    ...
    R_cl = calculate_rotation_matrices(
        Mrot, M_in_plane, M_sphere, queue, context
    )
    return R_cl
