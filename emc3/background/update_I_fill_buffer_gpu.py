...
from ..mapper import Mapper_cl_gpu_sparse
...
class Fill_buffers():
    ...
    def __init__(self, counts_m, m_min, m_max, model_shape, model_symmetry, queue, context):
        ...
        mf = cl.mem_flags
        self.a_cl = cl.Buffer(context, mf.READ_WRITE, self.a.nbytes)
        self.b_cl = cl.Buffer(context, mf.READ_WRITE, self.b.nbytes)
        self.index_m_cl = cl.Buffer(context, mf.READ_WRITE |
                mf.COPY_HOST_PTR, hostbuf=index_m)
    ...
    def fill_frame(self, P_r, K_i, B_i, w, C_i, N_sri):
        ...
        self.M_sri_cl = cl.Buffer(context, mf.READ_ONLY |
                mf.COPY_HOST_PTR, hostbuf=M_sri)

        self.P_r_cl = cl.Buffer(context, mf.READ_ONLY |
                mf.COPY_HOST_PTR, hostbuf=P_r)

        self.K_i_cl = cl.Buffer(context, mf.READ_ONLY |
                mf.COPY_HOST_PTR, hostbuf=K_i)

        self.B_i_cl = cl.Buffer(context, mf.READ_ONLY |
                mf.COPY_HOST_PTR, hostbuf=B_i)

        self.C_i_cl = cl.Buffer(context, mf.READ_ONLY |
                mf.COPY_HOST_PTR, hostbuf=C_i)
