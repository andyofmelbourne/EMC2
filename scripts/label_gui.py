...
class Application(QtWidgets.QMainWindow):
    def __init__(self, selection):
        ...
        print(f"type 's' to save True/False frames list to h5 file under {label}")
    ...
    def updateImage(self, init = False):
        ...
        self.imageitem.setImage(self.im**self.pow, autoRange = True, autoLevels = autoLevels, autoHistogramRange = True)
        self.update_border()
    ...
    def keyPressEvent(self, event):
        ...
        if key == QtCore.Qt.Key_Left :
            ...
            print('total number of selected frames:', np.sum(self.selection), 'index', self.currentIndex)
            self.update_border()
