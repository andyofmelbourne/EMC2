...
if __name__ == "__main__":
    ...
    # calculate c_n then write to file
    c_n = model_update_background_sparse.calculate_c_n(
        w_d, P_dr, mapper, C_i, N
    )

    fnam = f'class_{class_c.class_id}_c_n_{args.data_chunk}.h5'
    ...
    # get buffer size
    N_buffer = model_update_background_sparse.calculate_buffer_size(
        mapper.shape[0], rs_d, K_di_getter, device=opencl_stuff['device']
    )

    n_dsri, a_dsri, b_dsri = model_update_background_sparse.fill_buffers(
        mapper,
        K_di_getter,
        B_di,
        w_d,
    ...
    fnam = f'class_{class_c.class_id}_abn_{args.data_chunk}.h5'
    fnam = cachedir.joinpath(fnam)
    with h5py.File(fnam, 'w') as f:
        f['a_dsri'] = a_dsri
        f['b_dsri'] = b_dsri
