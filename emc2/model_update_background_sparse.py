...
def I_update(w_d, I_n, P_dr, K_di, B_di, M_sri, C_i, queue, context, **config):
    """
    """
    # this is absolute
    P_thresh = 1e-3
    ...
    c_n = np.zeros(I_n.size, dtype=float)
    r_chunk_size = 128
    ...
    for r0, r1, dr in r_iter:
        ...
        for _ in tqdm(range(1),
            ...
            c_n += np.bincount(
                N_sri.ravel(), t[:, :dr].ravel(), minlength=c_n.size
            )
