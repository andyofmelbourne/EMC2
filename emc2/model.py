...
def check_model_file(models_fnam, dimensions, model_length):
    ...
    with h5py.File(models_fnam, 'r') as f:
        ...
        for c, d in enumerate(dimensions):
            ...
            if len(m.shape) != d:
                print(f'file does not have models of \
                      correct dimesion {len(m.shape)} != {d}')
                return False

            if m.shape[0] != model_length:
                print(f'file does not have models of \
                      correct shape {m.shape[0]} != {model_length}')
                return False
...
def get_models(iteration, models_fnam, dimensions, model_length):
    ...
    if iteration == 0 or not \
       check_model_file(models_fnam, dimensions, model_length):
...
class Models():
    ...
    def __init__(self, no_gpu=True, **config):
        ...
        self.dimensions = utils.int_to_list(
            self.Nmodels,
            config['dimensions'],
            'dimensions'
        )

        self.I, self.w = get_models(
            config['iteration'],
            self.models_fnam,
            self.dimensions,
            self.model_length
